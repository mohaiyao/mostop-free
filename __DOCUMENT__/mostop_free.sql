...
Source Server         : 192.168.101.245（开发机）
Source Server Version : 50725
Source Host           : 192.168.101.245:3306
Source Database       : mostop_free
...
CREATE TABLE `mostop_admin` (
  ...
  `weixin` char(15) DEFAULT NULL COMMENT '微信',
  ...
  PRIMARY KEY (`userid`),
  KEY `created_at` (`created_at`) COMMENT '排序查询',
  KEY `name_created_at` (`name`,`created_at`) COMMENT '姓名查询',
  KEY `disabled_created_at` (`disabled`,`created_at`) COMMENT '状态查询'
...
CREATE TABLE `mostop_admin_login_log` (
  ...
  PRIMARY KEY (`logid`),
  KEY `created_at` (`created_at`) COMMENT '排序查询',
  KEY `ip_created_at` (`ip`,`created_at`) COMMENT 'IP 地址日志',
  KEY `userid_created_at` (`userid`,`created_at`) COMMENT '用户日志'
...
CREATE TABLE `mostop_admin_operate_log` (
  ...
  PRIMARY KEY (`logid`),
  KEY `created_at` (`created_at`) COMMENT '排序查询',
  KEY `url_created_at` (`url`(255),`created_at`) COMMENT '链接查询',
  KEY `userid_created_at` (`userid`,`created_at`) COMMENT '用户查询',
  KEY `ip_created_at` (`ip`,`created_at`) COMMENT 'IP 地址查询'
...
CREATE TABLE `mostop_menu` (
  ...
  `parent_ids` varchar(255) DEFAULT NULL COMMENT '所有父级 ID',
  `child_ids` text COMMENT '所有子级 ID',
...
CREATE TABLE `mostop_setting` (
  `app` varchar(15) NOT NULL COMMENT '应用',
  `parameter` varchar(32) NOT NULL COMMENT '参数名',
  `value` text COMMENT '参数值',
  PRIMARY KEY (`app`,`parameter`)
...
CREATE TABLE `mostop_user` (
  ...
  PRIMARY KEY (`userid`),
  KEY `created_at` (`created_at`) COMMENT '排序查询',
  KEY `username_created_at` (`username`,`created_at`) COMMENT '用户名查询排序'
) ENGINE=InnoDB AUTO_INCREMENT=10000 DEFAULT CHARSET=utf8 COMMENT='用户总表';
...
CREATE TABLE `mostop_user_0` (
  ...
  `salt` char(6) NOT NULL COMMENT '辅助密码',
  PRIMARY KEY (`userid`),
  KEY `username` (`username`) COMMENT '用户名查询'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户分表';
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `process_cycle_content_insert`()
BEGIN
